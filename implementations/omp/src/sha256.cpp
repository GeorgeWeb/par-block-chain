...
static inline uint32_t SHA256_F1(uint32_t x, uint32_t a, uint32_t b,
                                 uint32_t c) noexcept {
  return ((x >> a) | (x << ((sizeof(x) << 3u) - a))) ^
         ((x >> b) | (x << ((sizeof(x) << 3u) - b))) ^
         ((x >> c) | (x << ((sizeof(x) << 3u) - c)));
}

static inline uint32_t SHA256_F2(uint32_t x, uint32_t a, uint32_t b,
                                 uint32_t c) noexcept {
  return ((x >> a) | (x << ((sizeof(x) << 3u) - a))) ^
         ((x >> b) | (x << ((sizeof(x) << 3u) - b))) ^ (x >> c);
}
...
void SHA256::transform(const unsigned char *message, size_t block_nb) {
  ...
  for (size_t i = 0; i < block_nb; ++i) {
    ...
    for (size_t j = 16; j < 64; ++j) {
      w[j] = SHA256_F2(w[j - 2], 17, 19, 10) + w[j - 7] +
             SHA256_F2(w[j - 15], 7, 18, 3) + w[j - 16];
    }
    for (size_t j = 0; j < 8; ++j) {
      wv[j] = m_h[j];
    }
    for (size_t j = 0; j < 64; ++j) {
      t1 = wv[7] + SHA256_F1(wv[4], 6, 11, 25) + SHA2_CH(wv[4], wv[5], wv[6]) +
           sha256_k[j] + w[j];
...
void SHA256::final(unsigned char *digest) {
  ...
  size_t len_b;
  block_nb =
      (1u + ((SHA224_256_BLOCK_SIZE - 9u) < (m_len % SHA224_256_BLOCK_SIZE)));
...
std::string sha256(const std::string &input) {
  ...
  SHA256 ctx = SHA256();
  ctx.init();
  ctx.update(reinterpret_cast<const unsigned char *>(input.c_str()),
             input.length());
  ctx.final(digest);
