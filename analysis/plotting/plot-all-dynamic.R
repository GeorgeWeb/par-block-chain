TechniqueDIfficulty <- ResultsAllDynamic$Difficulty
TechniqueAvgTime <- as.numeric(as.character(ResultsAllDynamic$`Average Time Per Block`))
TechniqueName <- ResultsAllDynamic$Technique
TechniqueSpeedup <- as.numeric(as.character(ResultsAllDynamic$Speedup))
TechniqueEfficiency <- as.numeric(as.character(ResultsAllDynamic$Efficiency))
...
pAvgTimeBar <- ggplot(data = allTechniquesFrame, aes(x = Technique, y = `Average Time Per Block`, fill = Technique)) + 
  ...
  labs(x = "Technique", y = "Time (Log2 / ms)", title = "Log of Average Time Taken (ms) vs Difficulty", subtitle = "Comparison between all methods using dynamic number of threads calculation") +
...
pSpeedupBar <- ggplot(data = subset(allTechniquesFrame,Technique != c("0. Sequential")), aes(x = Technique, y = Speedup, fill = Technique)) + 
  ...
  theme(strip.text.x = element_text(size=12, angle=0),
        strip.text.y = element_text(size=16, face="bold"),
        strip.background = element_rect(color="black", fill="grey")) + 
  labs(x = "Technique", y = "Speedup factor", title = "Speedup vs Technique, Difficulty", subtitle = "Comparison between all parallel techniques using dynamic number of threads calculation") +
...
pEfficiencyBar <- ggplot(data = subset(allTechniquesFrame,Technique != c("0. Sequential")), aes(x = Technique, y = Efficiency, fill = Technique)) + 
  ...
  theme(strip.text.x = element_text(size=12, angle=0),
        strip.text.y = element_text(size=16, face="bold"),
        strip.background = element_rect(color="black", fill="grey")) + 
  labs(x = "Technique", y = "Efficiency factor", title = "Efficiency vs Technique, Difficulty", subtitle = "Comparison between all methods using dynamic number of threads calculation") +
  scale_x_discrete(breaks=c("1. OpenMP","2. Threads + Atomics", "3. Thread Pool + Futures"), 
                   labels = c("", "", ""))
